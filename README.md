...
flowchart LR
    ...
    subgraph OUTPUT["显示与执行"]
        OLED["OLED"]
        ACT["PWM 舵机 / LED / 蜂鸣器"]
    end

    GATEWAY["UART / DTU / 边缘网关"]

    SENSOR --> ADC --> FILTER --> SENSOR_TASK
    KEY --> KEY_TASK
    SENSOR_TASK --> DISPLAY_TASK --> OLED
    SENSOR_TASK --> CONTROL_TASK --> ACT
    KEY_TASK --> DISPLAY_TASK
    KEY_TASK --> CONTROL_TASK
